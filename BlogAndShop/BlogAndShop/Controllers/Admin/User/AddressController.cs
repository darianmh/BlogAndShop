...
{
    [AdminFilterName(AdminControllerNames.User, "آدرس ها")]
    public class AddressController : BaseAdminController
    {
        ...
        public async Task<IActionResult> Index(int page = 1, int count = 10)
        {
            ...
            var model = AdminModelHelper.GetIndexModel<AddressModel,Address>(all, page, count);
            return View(model);
        }
        ...
        public AddressController(IAddressService service, IAdminModelHelper adminModelHelper) : base(adminModelHelper)
        {
            _service = service;
        }
        #endregion
