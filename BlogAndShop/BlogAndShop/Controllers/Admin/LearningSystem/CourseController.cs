...
{
    [AdminFilterName(AdminControllerNames.LearningSystem, "دوره ها")]
    public class CourseController : BaseAdminController
    {
        ...
        public async Task<IActionResult> Index(int page = 1, int count = 10)
        {
            ...
            var model = AdminModelHelper.GetIndexModel<CourseViewModel, Course>(all, page, count);
            return View(model);
        }
        ...
        public CourseController(ICourseService service, IAdminModelHelper adminModelHelper) : base(adminModelHelper)
        {
            _service = service;
        }
        #endregion
