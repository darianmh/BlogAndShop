...
namespace BlogAndShop.Controllers.Admin.Forum
{
    ...
    public class ForumCommentController : BaseAdminController
    {
        ...
        public async Task<IActionResult> Index(int page = 1, int count = 10)
        {
            ...
            var model = new AdminListViewModel<ForumComment>(hasNext: all.TotalCount > page * count, hasPre: page > 1, items: all.List, page: page, count: all.List.Count, pagesCount: ((all.TotalCount - 1) / count) + 1);
            return View(model);
        }
        ...
        public ForumCommentController(IForumCommentService service) : base()
        {
            _service = service;
        }
        #endregion
